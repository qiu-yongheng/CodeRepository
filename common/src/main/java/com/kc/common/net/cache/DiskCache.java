...
import com.kc.common.util.AppInfoUtils;
 ...
 * @desc 磁盘缓存，KEY加密存储，可定制缓存时长 (框架使用磁盘缓存作为离线缓存)
 */
public class DiskCache implements ICache {
    ...
    public DiskCache(Context context, File diskDir, long diskMaxSize) {
        ...
        try {
            /** 创建一个DiskLruCache的实例 */
            cache = DiskLruCache.open(diskDir, AppInfoUtils.getVersionCode(context), 1, diskMaxSize);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public void put(String key, String value) {
        ...
        try {
            ...
            // 给数据添加尾部标识
            String content = value + TAG_CACHE.replace("createTime_v", String.valueOf(Calendar.getInstance().getTimeInMillis()))
                    .replace("expireMills_v", String.valueOf(cacheTime));
    ...
    private String getMd5Key(String key) {
        return EncryptUtils.encryptMD5ToString(key);
    }
